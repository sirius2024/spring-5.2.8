...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void initStrategies(ApplicationContext context) {
		...
		initHandlerMappings(context);//记住 映射关系建立
		initHandlerAdapters(context);//记住  负责具体逻辑的调用  controller调用 参数解析
		initHandlerExceptionResolvers(context);
		initRequestToViewNameTranslator(context);
		initViewResolvers(context);//记住 视图解析响应的
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		if (this.detectAllHandlerMappings) {// 零配置都走这里, 下面是老版本的不会走
			// Find all HandlerMappings in the ApplicationContext, including ancestor contexts.  6个
			//能够拿到实例 在ApplicationContext中查找所有handler映射，包括父类上下文.
			...
			try { // 如果设置为 false 那  Spring MVC就只会查找名为“handlerMapping”的bean，并作为当前系统的唯一的HandlerMapping
		...
		if (this.handlerMappings == null) {//默认的 不走
	 ...
	 * @throws Exception in case of any kind of processing failure  调用之前会初始化一些东西
	 */
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				// HandlerAdapter负责具体调用 根据handler找到对应的HandlerAdapter
				...
				//HandlerAdapter调用handle方法，进行具体Controller中方法的调用这个调用过程，关键点就在于参数的解析，其他都没什么技术含量。
	...
	protected HandlerAdapter getHandlerAdapter(Object handler) throws ServletException {
		...
		if (this.handlerAdapters != null) {
			for (HandlerAdapter adapter : this.handlerAdapters) {
				 ...
				 * SimpleControllerHandlerAdapter- BeanNameController
				 * HttpRequestHandlerAdapter - AreaController HttpRequestHandler
				 * 策略模式获取HandlerAdapter
				 */
