...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	...
	public final HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		//
		/**
		 * 根据请求的uri拿到对应的HandlerMethod对象, handler并不一定是HandleMethod 也看你是类本身
		 * 1、HandlerMethod条件:类上加Controller+requestMapping针对注解  handler=HandlerMehtod
		 * Object <- 类型不确定
		 * handler=BeanNameController
		 *
		 */
		...
		//此处把HandlerMethod对象封装成HandlerExecutionChain对象
		...
		if (hasCorsConfigurationSource(handler) || CorsUtils.isPreFlightRequest(request)) {
			//自定义的钩子方法获取跨域配置 UrlBasedCorsConfigurationSource.getCorsConfiguration
	...
	protected HandlerExecutionChain getHandlerExecutionChain(Object handler, HttpServletRequest request) {
		...
		//拿到url   new HandlerExecutionChain(handler)
