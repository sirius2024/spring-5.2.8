...
public final class MethodIntrospector {
	 ...
	 */
	/**
	 * 2、收集方法上面的@RequestMapping注解，把注解里面的配置信息封装到类里面，该类就是RequestMappingInfo类，
	 * 并且跟类上面的@RequestMapping注解封装类RequestMappingInfo合并，比如类上面是/common，方法上面是/queryUser。
	 * 这两者合并后就是/common/queryUser。这样的url才是我们需要的。合并完就是这样的url3、
	 * 然后建立method对象和对应的RequestMappingInfo的映射关系，把关系存放到map中。
	 */
	public static <T> Map<Method, T> selectMethods(Class<?> targetType, final MetadataLookup<T> metadataLookup) {
		...
		for (Class<?> currentHandlerType : handlerTypes) {
			...
			//此处是循环@Controller中的所有的方法然后判断有没有RequestMapping注解
			//循环currentHandlerType类的所有方法
			ReflectionUtils.doWithMethods(currentHandlerType, method -> {
				Method specificMethod = ClassUtils.getMostSpecificMethod(method, targetClass);
				//判断方法上面是否有@RequestMapping注解，如果有封装对象返回   specificMethod原始方法
				//此处调用的是外面的对象
				...
				if (result != null) {
					...
					if (bridgedMethod == specificMethod || metadataLookup.inspect(bridgedMethod) == null) {
						...
						//建立方法method和RequestMappingInfo的映射关系
