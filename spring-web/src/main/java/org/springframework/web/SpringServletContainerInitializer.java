...
/**
 * 此类会完成spring容器的启动
 * servlet容器会收集实现了WebApplicationInitializer这个接口的所有实例并调用onStartup方法，并且把收集到的实例作为入参传入此方法
 * WebApplicationInitializer实现了servlet规范，并且这个类会收集WebApplicationInitializer类软后放到onStartup方法使用
 */
...
public class SpringServletContainerInitializer implements ServletContainerInitializer {
	 ...
	 * @param servletContext the servlet context to be initialized  此方法是tomcat调的  调用onStartup方法
	...
	public void onStartup(@Nullable Set<Class<?>> webAppInitializerClasses, ServletContext servletContext)
		...
		//调用所有实例的onStartup方法，就完成了springmvc容器的启动
		for (WebApplicationInitializer initializer : initializers) {
			/**
			 * AbstractDispatcherServletInitializer会完成spring容器的启动
			 */
