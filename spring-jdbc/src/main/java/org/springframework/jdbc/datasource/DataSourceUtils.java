...
public abstract class DataSourceUtils {
	...
	public static Integer prepareConnectionForTransaction(Connection con, @Nullable TransactionDefinition definition)
		...
		// Apply specific isolation level, if any. 前一个隔离级别
		Integer previousIsolationLevel = null;
		if (definition != null && definition.getIsolationLevel() != TransactionDefinition.ISOLATION_DEFAULT) {//配置的和默认的不一致
			if (debugEnabled) {
				logger.debug("Changing isolation level of JDBC Connection [" + con + "] to " +
						definition.getIsolationLevel());
			}
			int currentIsolation = con.getTransactionIsolation(); //拿到当前隔离级别
			if (currentIsolation != definition.getIsolationLevel()) {//当前隔离级别和配置不一致
