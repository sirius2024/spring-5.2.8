...
public abstract class AbstractAdvisorAutoProxyCreator extends AbstractAutoProxyCreator {
	...
	/**
	 * Find all eligible Advisors for auto-proxying this class. 查找所有符合条件的顾问以自动代理此类。
	...
	protected List<Advisor> findEligibleAdvisors(Class<?> beanClass, String beanName) {
		//找到候选的切面,其实就是一个寻找有@Aspectj注解的过程，把工程中所有有这个注解的类封装成Advisor返回 看子类  AnnotationAwareAspectJAutoProxyCreator.findCandidateAdvisors
		List<Advisor> candidateAdvisors = findCandidateAdvisors();
		//判断候选的切面是否作用在当前beanClass上面，就是一个匹配过程。。现在就是一个匹配		获取适用增强器
